...
if (github_token == "") {
  ...
  # Clean up temp directory and close any stale connections from export
  unlink(local_export_dir, recursive = TRUE)
  
  # Close any open file connections from export phase
  open_cons <- showConnections(all = TRUE)
  if (nrow(open_cons) > 0) {
    for (i in seq_len(nrow(open_cons))) {
      con_num <- as.integer(rownames(open_cons)[i])
      if (con_num > 2) {
    ...
    if (httr::status_code(contents_response) != 200) {
      ...
      for (file_info in data_files) {
        ...
        tryCatch({
          ...
          # Close any connections from httr response
          httr::reset_handle(download_url)
          ...
          # Clean up - remove the downloaded object and file immediately
          rm(df)
          if (file.exists(local_path)) unlink(local_path)
          ...
          if (exists("local_path") && file.exists(local_path)) unlink(local_path)
        })
      }
      
      # Clean up download directory and close any open connections
      unlink(download_dir, recursive = TRUE)
      
      # Close all open file connections to avoid warnings
      open_cons <- showConnections(all = TRUE)
      if (nrow(open_cons) > 0) {
        for (i in seq_len(nrow(open_cons))) {
          con_num <- as.integer(rownames(open_cons)[i])
          if (con_num > 2) {  # Don't close stdin, stdout, stderr (0, 1, 2)
            tryCatch(close(getConnection(con_num)), error = function(e) NULL)
          }
        }
      }
...
# Final cleanup - close any remaining file connections before exit
final_cons <- showConnections(all = TRUE)
if (nrow(final_cons) > 0) {
  for (i in seq_len(nrow(final_cons))) {
    con_num <- as.integer(rownames(final_cons)[i])
    if (con_num > 2) {
      tryCatch(close(getConnection(con_num)), error = function(e) NULL)
    }
  }
